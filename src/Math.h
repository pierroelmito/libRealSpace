...
template <size_t N>
inline constexpr float PerlinNoise(float x, float y, const std::array<Vector2, N * N>& gradients)
{
	...
	const float x0y0 = Vector2Dot(gradients[iy0 * N + ix0], { srx0, sry0 });
	const float x1y0 = Vector2Dot(gradients[iy0 * N + ix1], { srx1, sry0 });
	const float x0y1 = Vector2Dot(gradients[iy1 * N + ix0], { srx0, sry1 });
	const float x1y1 = Vector2Dot(gradients[iy1 * N + ix1], { srx1, sry1 });
